...
def get_hyprctl_binds():
    while True:
        try:
            ...
            import re
            # Fix unquoted keycode strings
            raw_json = re.sub(r'"keycode":\s*([^,"]+)\s*,', lambda m: f'"keycode": "{m.group(1).strip()}",' if not m.group(1).strip().isdigit() and m.group(1).strip() not in ('true', 'false', 'null') else m.group(0), raw_json)
            # Fix allow_input_capture hijacking description with unquoted string
            raw_json = re.sub(r'"description":\s*"[^"]*",\s*"allow_input_capture":\s*(.*?)\s*,', r'"description": "\1", "allow_input_capture": false,', raw_json)
            # Fix key hijacking with false
            raw_json = re.sub(r'"key":\s*"false",\s*"keycode":\s*"([^"]+)",', r'"key": "\1", "keycode": "\1",', raw_json)
            
            binds = json.loads(raw_json)
            return binds
